...
class ColorConverter implements JsonConverter<Color, String> {
  ...
  @override
  Color fromJson(String jsonValue) {
    List<String>? values;
    if (jsonValue.isNotEmpty && jsonValue[0] == '#') {
      final hex = jsonValue.substring(1);
      final value = switch (hex.length) {
        // "#ff0"
        3 => int.parse(
        ...
        // "#ffff00"
        6 => int.parse('ff$hex', radix: 16),
        8 => int.parse(hex, radix: 16),
        _ => throw Exception("Can't parse the hex color '$jsonValue'")
      };
      return Color(value);
    }
    if (jsonValue.startsWith('rgb(')) {
      // "rgb(255, 255, 0)"
      values = jsonValue.substring(4, jsonValue.length - 2).split(',');
    } else if (jsonValue.startsWith('rgba(')) {
      // "rgba(255, 255, 0, 1)"
      values = jsonValue.substring(5, jsonValue.length - 2).split(',');
    }
    if (values != null) {
      return Color.fromARGB(
        values.length < 4 ? 255 : int.parse(values[3].trim()),
        int.parse(values[0].trim()),
        int.parse(values[1].trim()),
        int.parse(values[2].trim()),
      );
    }
    if (jsonValue.startsWith('hsl(')) {
      // "hsl(100, 50%, 50%)"
      values = jsonValue.substring(4, jsonValue.length - 2).split(',');
    } else if (jsonValue.startsWith('hsla(')) {
      // "hsla(100, 50%, 50%, 1)"
      values = jsonValue.substring(5, jsonValue.length - 2).split(',');
    }
    if (values != null) {
      return HSLColor.fromAHSL(
        jsonValue.length < 4 ? 1 : double.parse(jsonValue[3].trim()),
        double.parse(jsonValue[0].trim()),
        double.parse(jsonValue[1].trim().replaceFirst('%', '')) / 100.0,
        double.parse(jsonValue[2].trim().replaceFirst('%', '')) / 100.0,
      ).toColor();
    }
    // "yellow"
    final color = _colorNames[jsonValue.toLowerCase()];
    if (color != null) return color;

    throw Exception("Can't parse color: '$jsonValue'");
  }
